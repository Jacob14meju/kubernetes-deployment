...
def home():
   ...
   # Fetch VPCs
   vpc_data = [{"VPC ID": vpc["VpcId"], "CIDR": vpc["CidrBlock"]} for vpc in ["Vpcs"]]
  
   # Fetch Load Balancers
   lb_data = [{"LB Name": lb["LoadBalancerName"], "DNS Name": lb["DNSName"]} for lb in ["LoadBalancers"]]
  
   # Fetch AMIs (only owned by the account)
   ami_data = [{"AMI ID": ami["ImageId"], "Name": ami.get("Name", "N/A")} for ami in ["Images"]]
